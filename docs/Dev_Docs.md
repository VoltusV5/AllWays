...
# 4.6 Удаление ветки Локально
git branch -d <ветка>

# Удаляет ветку, если она полностью смёрджена.
# Принудительное удаление (осторожно — можно потерять коммиты!):
git branch -D <ветка>
...
Важно про жизненный цикл веток
•	Ветки не должны жить дольше 1–2 спринтов.
•	Чем дольше ветка живёт, тем выше риск забыть:
o	сливал ли ты её изменения,
o	есть ли там что-то полезное,
o	не конфликтует ли она с обновлённым develop.
•	Поэтому: сделал ветку → разработал фичу → отдал на просмотр → после одобрения влил в develop → удалил ветку
